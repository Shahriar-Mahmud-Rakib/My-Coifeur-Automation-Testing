...
BASE_URL = os.getenv("BASE_URL", "https://dev.mycoifeur.com.sa/en/register")

@pytest.mark.parametrize(
    "valid_email, valid_password",
    [
        ("testuser@mailinator.com", os.getenv("TEST_PASSWORD", "Test@1234!")),
        ("qa_1234567890@mailinator.com", os.getenv("TEST_PASSWORD", "Test@1234!"))
    ]
)
def test_flow_complete_cross_role_journey(page: Page, valid_email, valid_password):
    """Complete Cross-Role Journey

    Navigate to /en/register
    Click "Register as Salon Partner Tab"
    Enter Salon Brand Name (English): "Test E2E Salon"
    Enter Salon Brand Name (Arabic): ""
    Enter Commercial Registration (CR): "1010123456"
    Enter Owner Representative Name: "E2E Owner"
    Enter Salon Business Email: "salon.e2e@mycoifeur.com"
    Enter Salon Phone / Mobile: "501112222"
    Select City Dropdown: "Riyadh"
    Click "Submit Application Button"

    # TEST_DATA: valid_email, valid_password
    """
    page.goto(BASE_URL, wait_until="domcontentloaded", timeout=15000)
    expect(page).to_have_title(lambda t: bool(t and len(t) > 3))

    register_tab = page.locator('button[data-slot="tab-salon"]')
    register_tab.click()

    salon_name_en = page.locator('input[name="salonNameEn"]').first
    salon_name_ar = page.locator('input[name="salonNameAr"]').first
    cr_number = page.locator('input[name="crNumber"]').first
    owner_name = page.locator('input[name="ownerName"]').first
    business_email = page.locator('input[name="businessEmail"]').first
    business_phone = page.locator('input[name="businessPhone"]').first
    city_dropdown = page.locator('[name="cityId"]')

    salon_name_en.fill("Test E2E Salon")
    salon_name_ar.fill("")
    cr_number.fill("1010123456")
    owner_name.fill("E2E Owner")
    business_email.fill("salon.e2e@mycoifeur.com")
    business_phone.fill("501112222")

    city_dropdown.select_option("Riyadh")

    submit_button = page.locator('button:has-text("Submit Application Button")').filter(visible=True).first
    submit_button.click()
